...
#define CLOCK(...) dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);__VA_ARGS__; dispatch_semaphore_signal(_lock);
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"CTimer init error" reason:@"Use the designated initializer to init." userInfo:nil];
...
- (BOOL)repeats {
    CLOCK(BOOL repeat = _repeats); return repeat;
}

- (NSTimeInterval)timeInterval {
    CLOCK(NSTimeInterval t = _timeInterval) return t;
}

- (BOOL)isValid {
    CLOCK(BOOL valid = _valid) return valid;
}
